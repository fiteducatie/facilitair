<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('pin_meta', function (Blueprint $table) {
            $table->id();
            $table->foreignId('pin_id')->references('id')->on('pins')->onDelete('cascade');
            $table->string('school_name')->nullable();
            $table->string('school_location')->nullable();
            $table->date('datum_gebruikname')->nullable();
            $table->text('reden_bijzonderheid')->nullable();
            $table->text('meningen')->nullable();
            $table->text('primair_doel')->nullable();
            $table->text('bijzonderheden')->nullable();
            $table->text('betrokkenen')->nullable();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('pin_meta');
    }
};
